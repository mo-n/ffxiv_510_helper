...
function Armorys(props: Props) {
  ...
  return (
    <div>
      ...
      <div className="flex">
        <div className="flex justify-around select-none py-2 w-10 mr-2 text-gray-800 border-gray-300 border-solid border-r box-content flex-wrap flex-none">
        ...
        <div className="flex-auto py-4 select-none flex flex-wrap">
          {armoryChests.map(({ Icon, id, order }) => {
            return (
              <div className={`flex flex-1 order-${order}`} key={id}>
                <Icon className="w-8 h-8 mr-2 cursor-pointer rounded-md fill-current"
                  onClick={() => {
                    store.setArmor(currentJob, id, store.getArmor(currentJob, id)! + 1);
                  }}
                />
                <InputNumber
                  className="w-20"
